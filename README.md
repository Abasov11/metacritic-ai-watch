...
- **Каталог** — <https://skytec.213-32-66-46.nip.io/> — список игр, живой поиск, фильтр
  по платформам, сортировка. Бейдж «резюме» отмечает игры, где модель уже написала
  пересказ отзывов; чекбокс «только с резюме отзывов» оставит на странице только их.
- **Карточка с содержимым** —
  <https://skytec.213-32-66-46.nip.io/game/onimusha-way-of-the-sword> — резюме критиков
  и игроков, летсплей с заключением по расшифровке речи, похожие игры с общими тегами.
- **Мониторинг** — <https://skytec.213-32-66-46.nip.io/monitor> — состояние воркеров,
  прогресс обхода, счётчики и бюджет за сегодня, живой лог событий по SSE.

У большинства игр в каталоге резюме нет — это не поломка: Metacritic заводит страницу
до релиза, отзывов там ещё нет, и сервис честно пишет «Отзывов пока нет».
...
### Файл кук использует только один процесс

yt-dlp **переписывает** файл кук обновлённой сессией после каждого запуска. Если в него
пишут двое — сервис и запущенная руками команда, или сервис и браузер, — сессия ломается
и возвращается «Sign in to confirm you're not a bot».

Поэтому:

- каждый вызов yt-dlp в коде берёт межпроцессный замок `fcntl.flock` на
  `<файл кук>.lock`, так что сервис и CLI не ходят в YouTube одновременно;
- замок защищает от одновременности, но не от третьего участника: **не открывайте
  youtube.com этим аккаунтом**, пока куки в работе;
- при долгом ручном пересчёте (`--refresh-letsplays` по всей базе) сервис лучше
  остановить: `sudo systemctl stop metacritic-ai-watch`, потом запустить обратно.

### Запрашивается ровно одна дорожка субтитров

Раньше запрашивались `en` и `ru` сразу. Для англоязычного ролика русская дорожка — это
машинный перевод той же речи, то есть лишний запрос ради худшего текста; именно он
приносил `HTTP Error 429: Too Many Requests` и ронял всю стадию, хотя английские
субтитры были доступны.

Теперь сначала запрашивается список дорожек, из него выбирается **одна**: ручные `en` →
авто `en` → ручные/авто `ru` → любая доступная (модель прочитает и другой язык). Если
дорожка не скачалась, пробуется следующая, а не падает вся стадия. На 429 и 5xx —
паузы 15/45/120 секунд в пределах бюджета `YOUTUBE_TIMEOUT`; если и после них отказ,
записывается «YouTube: слишком много запросов, повтор в следующем обходе», и запись
остаётся несвежей, чтобы следующий обход попробовал снова. Между роликами — пауза
`YOUTUBE_DELAY` (5 с).

Повторить расшифровку для роликов, которые нашлись, но не прочитались:
`python -m app.crawler --refresh-letsplays`.

Прогон по базе после этих правок: из 28 записей **15 получили расшифровку** (медиана
~32 тыс. символов, максимум 176 тыс.), 9 — у роликов субтитров нет вовсе, 4 упёрлись в
429 и будут повторены следующим обходом. Заключения по 17 роликам обошлись в $0.007.
...
| `MANUAL_RUN_COOLDOWN_MINUTES` | `10` | Пауза между ручными запусками обхода; плановых не касается |
| `LLM_DAILY_BUDGET_USD` | `1.0` | Дневной лимит трат на модель; `0` — без лимита |
...
371 тест, в сеть не ходят: HTTP мокается через `httpx.MockTransport`, парсеры
...
| `POST /monitor/run`: проверка `Sec-Fetch-Site`/`Origin` (кросс-сайт → 403), глобальный cooldown 10 мин (→ 429), дневной лимит трат на модель | `app/main.py`, `app/llm.py` |
...
- **Нет аутентификации на `/monitor`.** Это витрина тестового задания, кнопка запуска
  должна быть видна проверяющему. **Кнопку может нажать кто угодно, кто дошёл до
  публичного адреса** — в том числе `curl -X POST /monitor/run` без браузера: проверка
  источника отклоняет запрос с чужим `Origin`/`Sec-Fetch-Site`, но запрос вообще без
  этих заголовков (любой консольный клиент) пропускается, иначе сломались бы
  healthcheck и CLI.
  Поэтому ограничен не доступ, а ущерб: глобальный cooldown ручных запусков
  (`MANUAL_RUN_COOLDOWN_MINUTES`, 10 мин), дневной лимит трат на модель
  (`LLM_DAILY_BUDGET_USD`, $1 — при исчерпании модель не вызывается, игры остаются к
  повтору) и отдельный лимит на обновление одной игры (раз в минуту). Худшее, что
  получится сделать, — один лишний обход в 10 минут в пределах дневного бюджета.
  Если сервис станет не демонстрационным — `auth_basic` на `/monitor*` в nginx (одна
  строка) или токен в заголовке.
